...
export const unlink: Command = {
  ...
  async execute(interaction) {
    ...
    if (playerOpt) {
      ...
      await interaction.editReply(`✅ Unlinked **${target.displayName}**${oldDiscordId ? ' and removed their roles' : ''}.`);
      return;
    }
    ...
    await interaction.editReply(
      `✅ Unlinked you from **${mine.displayName}**. Run /link to connect the right player.`,
    );
  },
